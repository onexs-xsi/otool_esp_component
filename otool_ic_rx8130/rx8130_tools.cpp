...
static void IRAM_ATTR rx8130_gpio_isr_handler(void* arg)
{
    // 这是ISR处理函数，这里先简单处理
    // 后续会添加任务系统的处理逻辑
    rx8130_tools* rtc = (rx8130_tools*)arg;
    if (rtc != NULL) {
        // 清除中断标志
        rtc->clearIrqFlags();
    ...
    if (rtc->_interrupt_callback != nullptr) {
            ...
            xTaskCreate(rx8130_tools::callback_task, "rtc_callback", 2048, (void*)rtc, 5, NULL);
        }
    }
}
...
bool rx8130_tools::init(i2c_master_bus_handle_t bus, i2c_master_dev_handle_t *i2c_device, gpio_num_t int_pin)
{
    if (i2c_device == NULL) {
        return false;
    }
...
bool rx8130_tools::init(i2c_bus_handle_t bus, i2c_bus_device_handle_t *i2c_device, gpio_num_t int_pin)
{
    if (i2c_device == NULL) {
        return false;
    }
...
void rx8130_tools::setTime(struct tm* time)
{
    // 记录当前时间和新时间，计算偏移量
...
void rx8130_tools::getTime(struct tm* time)
{
    uint8_t date[7];
...
void rx8130_tools::clearIrqFlags()
{
    writeRegister8(RX8130_REG_FLAG, 0);
}

void rx8130_tools::disableIrq()
{
    writeRegister8(RX8130_REG_CTRL0, 0);
}

void rx8130_tools::setAlarmIrq(struct tm* time)
{
    uint8_t buf = 0;
...
void rx8130_tools::setTimerIrq(uint16_t seconds)
{
...
void rx8130_tools::setInterruptCallback(task_callback_t callback)
{
    _interrupt_callback = callback;
}

void rx8130_tools::clearInterruptCallback()
{
    _interrupt_callback = nullptr;
}

void rx8130_tools::enableInterrupt()
{
...
void rx8130_tools::disableInterrupt()
{
    if (_int_pin != GPIO_NUM_NC) {
...
uint8_t rx8130_tools::readRegister8(uint8_t reg)
{
    uint8_t value;
...
void rx8130_tools::writeRegister8(uint8_t reg, uint8_t value)
{
    uint8_t buf[1] = {value};
    writeRegister(reg, buf, 1);
}

void rx8130_tools::readRegister(uint8_t reg, uint8_t* buf, uint8_t len)
{
...
void rx8130_tools::writeRegister(uint8_t reg, uint8_t* buf, uint8_t len)
{
    if (_i2c_bus_device_handle) {
...
bool rx8130_tools::setupInterrupt(gpio_num_t int_pin)
{
    _int_pin = int_pin;
...
// 静态回调任务函数
void rx8130_tools::callback_task(void* param)
{
    rx8130_tools* rtc = (rx8130_tools*)param;
...
// 获取当前RTC时间（Unix时间戳）
time_t rx8130_tools::getCurrentTime()
{
...
// 延时执行任务
rtc_task_handle rx8130_tools::scheduleTask(uint32_t delay_seconds, task_callback_t callback, const std::string& description)
{
...
// 指定时间执行任务
rtc_task_handle rx8130_tools::scheduleTaskAt(struct tm* target_time, task_callback_t callback, const std::string& description)
{
...
// 重复执行任务
rtc_task_handle rx8130_tools::scheduleRepeatingTask(uint32_t interval_seconds, task_callback_t callback, const std::string& description)
{
    time_t current_time = getCurrentTime();
...
// 指定时间开始的重复任务
rtc_task_handle rx8130_tools::scheduleRepeatingTaskAt(struct tm* start_time, uint32_t interval_seconds, task_callback_t callback, const std::string& description)
{
...
// 取消任务
bool rx8130_tools::cancelTask(rtc_task_handle handle)
{
...
// 取消所有任务
void rx8130_tools::cancelAllTasks()
{
    for (auto& task : _tasks) {
        task->is_active = false;
    }
...
// 获取活跃任务数量
size_t rx8130_tools::getActiveTaskCount()
{
    size_t count = 0;
...
// 更新下一个闹钟时间
void rx8130_tools::updateNextAlarm()
{
    time_t current_time = getCurrentTime();
...
// 处理触发的任务
void rx8130_tools::processTriggeredTasks()
{
    processTriggeredTasksWithTolerance(0); // 精确匹配，无容错
}

// 处理触发的任务（带容错）
void rx8130_tools::processTriggeredTasksWithTolerance(uint32_t tolerance_seconds)
{
    time_t current_time = getCurrentTime();
...
void rx8130_tools::recalculateAllTasks(time_t time_offset)
{
    for (auto& task : _tasks) {
        if (task->is_active) {
...
// 获取任务信息
bool rx8130_tools::getTaskInfo(rtc_task_handle handle, rtc_task_info& info)
{
    for (const auto& task : _tasks) {
        if (task->task_id == handle.task_id && task->is_active) {
            info.handle = handle;
...
// 获取所有任务信息
std::vector<rtc_task_info> rx8130_tools::getAllTasksInfo()
{
...
// 获取所有活跃任务句柄
std::vector<rtc_task_handle> rx8130_tools::getActiveTaskHandles()
{
...
// 打印任务状态（调试用）
void rx8130_tools::printTasksStatus()
{
    time_t current_time = getCurrentTime();
    printf("\n=== RTC Tasks Status ===\n");
...
void rx8130_tools::manualTriggerCheck(uint32_t tolerance_seconds)
{
    time_t current_time = getCurrentTime();
