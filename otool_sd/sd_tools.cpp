...
static const char *TAG = "sd_tools";
...
sd_tools::sd_tools() 
    : initialized(false), mounted(false), card(nullptr), current_mode(SD_MODE_MMC_1BIT),
      ...
      mmc_clk_pin(GPIO_NUM_10), mmc_cmd_pin(GPIO_NUM_7), mmc_d0_pin(GPIO_NUM_8),
      mmc_d1_pin(GPIO_NUM_NC), mmc_d2_pin(GPIO_NUM_NC), mmc_d3_pin(GPIO_NUM_NC),
      mmc_internal_pullup(true),
      spi_cs_pin(GPIO_NUM_50), spi_mosi_pin(GPIO_NUM_7), spi_miso_pin(GPIO_NUM_8),
      spi_clk_pin(GPIO_NUM_10), spi_host(SPI3_HOST), spi_max_transfer_sz(2048)
{
    ESP_LOGD(TAG, "sd_tools constructor called");
}

sd_tools::sd_tools(sd_test_mode_t mode) : sd_tools()
{
    this->mode = mode;
    ESP_LOGD(TAG, "sd_tools constructor called with mode: %s", get_mode_description(mode));
}
...
void sd_tools::set_mode(sd_test_mode_t mode)
{
    if (initialized) {
    ...
    this->mode = mode;
    ESP_LOGD(TAG, "Mode set to: %s", get_mode_description(mode));
}
...
void sd_tools::set_mmc_pins(gpio_num_t clk_pin, gpio_num_t cmd_pin, gpio_num_t d0_pin, bool internal_pullup)
{
    if (initialized) {
        ESP_LOGW(TAG, "Cannot change pins while initialized");
        return;
    }
    ...
    ESP_LOGD(TAG, "MMC pins set - CLK: %d, CMD: %d, D0: %d, pullup: %s", 
             clk_pin, cmd_pin, d0_pin, internal_pullup ? "enabled" : "disabled");
}

void sd_tools::set_mmc_4bit_pins(gpio_num_t clk_pin, gpio_num_t cmd_pin, gpio_num_t d0_pin,
                                 ...
                                 bool internal_pullup)
{
    ...
    this->mmc_internal_pullup = internal_pullup;
    ESP_LOGD(TAG, "MMC 4-bit pins set - CLK: %d, CMD: %d, D0: %d, D1: %d, D2: %d, D3: %d, pullup: %s", 
             clk_pin, cmd_pin, d0_pin, d1_pin, d2_pin, d3_pin, internal_pullup ? "enabled" : "disabled");
}

void sd_tools::set_spi_pins(gpio_num_t cs_pin, gpio_num_t mosi_pin, gpio_num_t miso_pin, 
                            gpio_num_t clk_pin, spi_host_device_t host, uint32_t max_transfer_sz)
{
    ...
    ESP_LOGD(TAG, "SPI pins set - CS: %d, MOSI: %d, MISO: %d, CLK: %d, host: %d, max_transfer: %lu", 
